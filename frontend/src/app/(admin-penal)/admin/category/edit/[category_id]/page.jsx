...
function CategoryEdit({ params }) {
  ...
  async function getFormData() {
    const getcat = await getCategory(category_id);
    setCategory(getcat.data);
  }
  ...
  const handlerSubmit = (e) => {
    ...
    if (nameRef.current.value === "" || slugRef.current.value === "") {
      ...
      axioIsnstance
        .put(`category/update/${category_id}`, formData, {
          headers: {
            Authorization: token, 
          }, 
        })
        ...
        <div>
          ...
          <div className="flex items-center gap-5">
            ...
            <div className="w-24 h-24 border border-gray-200 rounded-xl flex items-center justify-center text-gray-400 text-xs bg-gray-50">
              <img
                src={`${process.env.NEXT_PUBLIC_API__BASE_URL}/image/category/${category.image}`}
                alt={category.name}
